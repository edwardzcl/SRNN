...
def mem_update(ops1, ops2, x, mem, spike):
    input_sops = torch.sum(torch.abs(x)) * len(ops1.weight)
    feedback_sops = torch.sum(torch.abs(spike)) * len(ops2.weight)
    total_sops = input_sops + feedback_sops
    mem = mem * decay * (1. - spike) + ops1(x) + ops2(spike) / 10. #ops2(mem)
    spike = act_fun(mem) # act_fun : approximation firing function
    return mem, spike, total_sops
